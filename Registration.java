...
/***********ADD CODE HERE***********/                
    ...
    public String selectAllNames(String tableName){     
        ...
        List<String> validTableNames = Arrays.asList("competitor", "teacher");
        ...
        try {
            ...
            results = statement.executeQuery("SELECT lname, fname FROM " + tableName);
            while (results.next()) {
                builder.append(results.getString("lname")).append(", ")
                        .append(results.getString("fname")).append("\n");
            }
            if (builder.length() > 0) {
    ...
    public void insertNewCompetitor(String id, String lName, String fName, int age, String instrument, String teacherID){
        ...
        // The other parts of the method have already been provided
        String insertSql = "INSERT INTO competitor (competitorid, lname, fname, age, instrument, teacherid) " +
                           "VALUES (?, ?, ?, ?, ?, ?) " + "ON CONFLICT (competitorid) DO UPDATE SET " +
                           "lname = EXCLUDED.lname, " + "fname = EXCLUDED.fname, " + "age = EXCLUDED.age, " +
                           "instrument = EXCLUDED.instrument, " + "teacherid = EXCLUDED.teacherid";
        
        try (PreparedStatement preparedStatement = dbConnect.prepareStatement(insertSql)) {
...
// Used to ensure that any new students are connected to an existing teacher    
    ...
    public void deleteCompetitor(String id){
        String deleteSql = "DELETE FROM competitor WHERE competitorid = ?";
        try (PreparedStatement preparedStatement = dbConnect.prepareStatement(deleteSql)) {
            preparedStatement.setString(1, id);
            preparedStatement.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
